							...
							<div class="form-group" id="${i.index}">
								...
								<label class="control-label"
									for="groupEventPassCategoryId${i.index}">${passCategory.passCategoryNameShort}
									 <span	class="badge badge-warning">$${passCategory.passPrice}</span><c:choose>
										...
										<c:otherwise>
											<a tabindex="0" role="button" data-toggle="popover"
												data-container="body" data-animation="true"
												data-trigger="focus" title="Are there any limits on my ticket purchase?"
												data-placement="top"
			...
			<div class="alert">
				<c:if test="${fn:length(paymentTransactions) gt 0}">
					...
					<table class='table table-striped table-bordered table-condensed'>
						...
						<c:forEach items="${paymentTransactions}" var="paymentTransaction"
							varStatus="p">

							<tr>
								<td>${paymentTransaction.totalNumberOfProducts}</td>
								<td>$${paymentTransaction.transactionAmount}</td>
								<td>${paymentTransaction.paymentStatus}
								<c:choose>
								<c:when test="${not empty paymentTransaction.transactionExpiryDateTime and paymentTransaction.paymentStatus == 'EXPIRED'}">
										<a tabindex="0" role="button" data-toggle="popover"
											data-container="body" data-animation="true"
											data-trigger="focus" title="When did the transaction Expire?"
